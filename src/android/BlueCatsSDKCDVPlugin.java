...
public class BlueCatsSDKCDVPlugin extends CordovaPlugin {
    ...
    public static final String ACTION_GET_ALL_BEACONS = "getAllBeacons";
    public static final String ACTION_GET_ALL_SITES = "getAllSites";
    ...
    public boolean execute(final String action, final JSONArray args, final CallbackContext callbackContext) throws JSONException {
        ...
        try {
            if (action.equalsIgnoreCase(ACTION_START_PURRING_WITH_APP_TOKEN)) {
                ...
                return true;
            } else if (action.equalsIgnoreCase(ACTION_GET_ALL_BEACONS)) {
                cordova.getThreadPool().execute(new Runnable() {
                    @Override
                    public void run() {


                        BCAccountManager.getInstance().getVerifiedApp().getSites(false, new BCAppCallback() {
                            @Override
                            ...
                            @Override
                            public void onDidLoadBeacons(List<BCBeacon> bcBeacons) {
                                try {
                                    sendJsonObject(new JSONObject(getGson().toJson(bcBeacons)), callbackContext, false);
                                } catch (JSONException e) {
                                    e.printStackTrace();
                                    sendErrorMessage(callbackContext, "Couldnt turn sites into json. ");
                                }
                            }

                            @Override
                            public void onDidLoadSites(List<BCSite> bcSites) {

                            }

                            @Override
                            public void onDidGetAppToken(String s) {
                            ...
                            public void onDidFailWithError(BCError bcError) {
                                sendErrorMessage(callbackContext, "Couldnt load beacons. ");
                            }
                        });
                    }
                });
                return true;
            } else if (action.equalsIgnoreCase(ACTION_GET_ALL_SITES)) {
                cordova.getThreadPool().execute(new Runnable() {
                    @Override
                    public void run() {
                        cordova.getThreadPool().execute(new Runnable() {
                            ...
                            public void run() {
                                ...
                                BCAccountManager.getInstance().getVerifiedApp().getBeacons(false, new BCAppCallback() {
                                    @Override
                                    public void onDidUpdateApp() {
                                    ...
                                    public void onDidLoadSites(List<BCSite> bcSites) {
                                        try {
                                            sendJsonObject(new JSONObject(getGson().toJson(bcSites)), callbackContext, false);
                                        } catch (JSONException e) {
                                            e.printStackTrace();
                                            sendErrorMessage(callbackContext, "Couldnt turn sites into json. ");
                                        }
